...
const screen_theme_t THEME_DEFAULT = {
	.border_fg      = COLOR_CYAN,
...
const screen_theme_t THEME_DARK = {
	.border_fg      = COLOR_BLUE,
	.border_bg      = COLOR_DEFAULT,
	.title_fg       = COLOR_BRIGHT_WHITE,
	.title_bg       = COLOR_DEFAULT,
	.text_fg        = COLOR_WHITE,
...
const screen_theme_t THEME_LIGHT = {
	.border_fg      = COLOR_BLUE,
	.border_bg      = COLOR_DEFAULT,
...
const screen_theme_t THEME_MONO = {
...
static int fg_code(screen_color_t c)
{
	...
	case COLOR_BRIGHT_BLACK:  return 90;
	case COLOR_BRIGHT_RED:    return 91;
	case COLOR_BRIGHT_GREEN:  return 92;
	case COLOR_BRIGHT_YELLOW: return 93;
	case COLOR_BRIGHT_BLUE:   return 94;
	case COLOR_BRIGHT_MAGENTA:return 95;
	case COLOR_BRIGHT_CYAN:   return 96;
	case COLOR_BRIGHT_WHITE:  return 97;
	default:                  return 39;
	}
}
...
static int bg_code(screen_color_t c)
{
	...
	case COLOR_BRIGHT_BLACK:  return 100;
	case COLOR_BRIGHT_RED:    return 101;
	case COLOR_BRIGHT_GREEN:  return 102;
	case COLOR_BRIGHT_YELLOW: return 103;
	case COLOR_BRIGHT_BLUE:   return 104;
	case COLOR_BRIGHT_MAGENTA:return 105;
	case COLOR_BRIGHT_CYAN:   return 106;
	case COLOR_BRIGHT_WHITE:  return 107;
	default:                  return 49;
	}
}
...
void screen_init(screen_t *scr, int width, int height)
{
	...
	if (height > SCREEN_MAX_HEIGHT) height = SCREEN_MAX_HEIGHT;
...
void screen_clear(screen_t *scr)
{
	for (int r = 0; r < scr->height; r++) {
		...
		for (int c = 0; c < scr->width; c++) {
			scr->colors[r][c] = MAKE_COLOR(COLOR_DEFAULT, COLOR_DEFAULT);
...
void screen_putc(screen_t *scr, char c)
{
	...
	if (scr->cursor_row < scr->height && scr->cursor_col < scr->width) {
		...
		scr->attrs[scr->cursor_row][scr->cursor_col] = scr->current_attr;
		scr->cursor_col++;
	}
}
...
void screen_vline(screen_t *scr, int row, int col, int len, char c)
{
	for (int i = 0; i < len && row + i < scr->height; i++) {
		...
		scr->colors[row + i][col] = MAKE_COLOR(scr->current_fg, scr->current_bg);
...
void screen_box(screen_t *scr, int row, int col, int width, int height)
{
	...
	if (row < scr->height && col < scr->width) {
		...
		scr->colors[row][col] = MAKE_COLOR(scr->current_fg, scr->current_bg);
	}
	if (row < scr->height && col + width - 1 < scr->width) {
		scr->cells[row][col + width - 1] = BOX_TR;
		scr->colors[row][col + width - 1] = MAKE_COLOR(scr->current_fg, scr->current_bg);
	}
	if (row + height - 1 < scr->height && col < scr->width) {
		scr->cells[row + height - 1][col] = BOX_BL;
		scr->colors[row + height - 1][col] = MAKE_COLOR(scr->current_fg, scr->current_bg);
	}
	if (row + height - 1 < scr->height && col + width - 1 < scr->width) {
		scr->cells[row + height - 1][col + width - 1] = BOX_BR;
		scr->colors[row + height - 1][col + width - 1] = MAKE_COLOR(scr->current_fg, scr->current_bg);
	}
	...
	for (int c = col + 1; c < col + width - 1 && c < scr->width; c++) {
		if (row < scr->height) {
			...
			scr->colors[row][c] = MAKE_COLOR(scr->current_fg, scr->current_bg);
		}
		if (row + height - 1 < scr->height) {
			scr->cells[row + height - 1][c] = BOX_H;
			scr->colors[row + height - 1][c] = MAKE_COLOR(scr->current_fg, scr->current_bg);
		}
	}
	...
	for (int r = row + 1; r < row + height - 1 && r < scr->height; r++) {
		if (col < scr->width) {
			...
			scr->colors[r][col] = MAKE_COLOR(scr->current_fg, scr->current_bg);
		}
		if (col + width - 1 < scr->width) {
			scr->cells[r][col + width - 1] = BOX_V;
			scr->colors[r][col + width - 1] = MAKE_COLOR(scr->current_fg, scr->current_bg);
		}
	}
}
...
void screen_render(screen_t *scr)
{
	...
	for (int r = 0; r < scr->height; r++) {
		...
		pos += snprintf(out + pos, sizeof(out) - pos, "\033[%d;1H", r + 1);
		...
		for (int c = 0; c < scr->width; c++) {
			...
			if (fg != last_fg || bg != last_bg || attr != last_attr) {
				pos += snprintf(out + pos, sizeof(out) - pos, "\033[0");
				
				if (attr & ATTR_BOLD)
					pos += snprintf(out + pos, sizeof(out) - pos, ";1");
				if (attr & ATTR_DIM)
					pos += snprintf(out + pos, sizeof(out) - pos, ";2");
				if (attr & ATTR_UNDERLINE)
					pos += snprintf(out + pos, sizeof(out) - pos, ";4");
				if (attr & ATTR_REVERSE)
					pos += snprintf(out + pos, sizeof(out) - pos, ";7");
				
				pos += snprintf(out + pos, sizeof(out) - pos, ";%d;%dm",
				               fg_code(fg), bg_code(bg));
